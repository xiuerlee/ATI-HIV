...
#
# Fixed parameters based on the earlier/original fixed-parameter background,
# with only viral production fixed to p = 8000:
#   p       = 8000
#   delta0  = 1.4
#   c       = 23
#   d_C     = 0.75
...
# Fixed constants from the earlier/original fixed-parameter background.
# p is fixed to 8000.
# K_C and K_kill are updated based on the observed CD8 scale and model-scale reasoning.
P_FIXED <- 8000
DELTA0_FIXED <- 1.4
C_FIXED <- 23
D_C_FIXED <- 0.75
...
out_dir <- file.path(
  DATA_DIR,
  "model_selection_fixed_p8000_original_params_KC1000_Kkill2_CD8_Cbase_four_mechanisms",
  run_tag
)
...
run_settings <- tibble(
  ...
  value = c(
    ...
    "T,L,I,V,C; no exhaustion compartment; saturated CD8 killing; original fixed parameters except p fixed at 8000; new C_base_group CD8 equation",
    "a_L, k_C, xi_C, rho_C",
...
p_aicc <- aicc_table_ms %>%
  ...
  theme_bw() +
  labs(
    title = "Model selection by AICc: original fixed params + fixed p = 8000",
    x = "Model",
