...
# ============================================================
# Total-CD8 exhaustion model selection script
# ============================================================
# Current model:
#   T, L, I, V, CF, CE
#   CF = functional CD8
#   CE = exhausted / dysfunctional CD8
#   observed total CD8 = CF + CE
#
# Group-difference model selection tests whether early vs late groups differ in:
#   1) a_L      : latent-cell activation
#   2) k_C      : CD8 cytolytic killing
#   3) xi_C     : CD8 non-cytolytic suppression
#   4) rho_C    : CD8 proliferation
#   5) chi_C    : CD8 exhaustion
#
# Sign convention:
...
# If you do not have exhaustion-marker data, do not estimate the initial
# exhausted fraction. Keep it fixed in the first version.
FRAC_E0_FIXED <- 0

# Fixed half-saturation constants for CD8 activation/exhaustion.
# These have the same internal scale as I.
# If sensitivity checks are needed later, change these two constants only.
K_C_FIXED <- 30
K_CHI_FIXED <- 600
...
out_dir <- file.path(
  "../Rdata/model_selection_results_total_CD8_exhaustion",
  run_tag
)
...
ati_model <- function(t, state, pars) {
  with(as.list(c(state, pars)), {
    ...
    CD8_killing_rate <- k_C_G * CF

    p_eff <- p_G / (1 + xi_C_G * CF)
...
init_table <- read.csv("../Rdata/init_table_I0_replaced_with_CD8.csv")
...
# ============================================================
# k_C and xi_C are linear effects of CD8 count.
# Because CD8 is around 10^6 cells/mL, these parameters must be small.
#
# k_C units: day^-1 per cell/mL
# xi_C units: mL/cell
# rho_C, chi_C units: day^-1
...
# ============================================================
# 9. Add initial CD8 to initial table
# ============================================================
# Priority:
#   1) individual CD8 at time 0
#   2) individual median CD8
#   3) group median CD8
#   4) overall median CD8
#
# Initial exhausted CD8 is fixed to zero in the first version:
#   CE0 = 0
#   CF0 = total CD8_0
...
init_fit <- init_raw %>%
  ...
  mutate(
    ...
    CD8_0_source = case_when(
      is.finite(CD8_count_id) ~ CD8_0_source_id,
      !is.finite(CD8_count_id) & is.finite(CD8_count_group) ~ "group_median",
      TRUE ~ "overall_median"
    ),
    frac_E0 = FRAC_E0_FIXED,
    CF0 = (1 - frac_E0) * CD8_0,
    CE0 = frac_E0 * CD8_0
  ) %>%
  filter(
...
write.csv(
  init_fit,
  file.path(out_dir, "init_table_with_CD8_used.csv"),
  row.names = FALSE
)
...
# zchi : chi_C early/late difference
#
# There are 2^5 = 32 candidate models.

make_model_name <- function(za, zk, zxi, zrho, zchi) {
  ...
  labels <- c(
    ...
    ifelse(zchi == 1, "chi", NA_character_)
  )
...
candidate_models <- expand.grid(
  ...
  as_tibble() %>%
  mutate(
    n_group_effects = za + zk + zxi + zrho + zchi,
    model = pmap_chr(
      list(za, zk, zxi, zrho, zchi),
      make_model_name
    )
  ) %>%
  arrange(n_group_effects, za, zk, zxi, zrho, zchi) %>%
  select(model, za, zk, zxi, zrho, zchi)
...
simulate_one <- function(id_row, pars, times, model_flags) {
  ...
  chi_C_G <- as.numeric(pars["chi_C"]) *
    exp(as.numeric(model_flags["zchi"]) * as.numeric(pars["theta_chi"]) * G)
  ...
  CF0 <- max(as.numeric(id_row$CF0), EPS)
  CE0 <- max(as.numeric(id_row$CE0), 0)

  # Homeostatic CD8 input scaled to each individual's initial functional CD8.
  # This avoids adding one extra free source parameter.
  s_C_G <- as.numeric(pars["d_F"]) * CF0
  ...
  sim %>%
    mutate(
      ...
      CE0 = CE0,
      CD8_0 = CF0 + CE0,
      CD8_0_source = id_row$CD8_0_source,
      a_L_G = a_L_G,
      p_G = p_G,
      k_C_G = k_C_G,
      xi_C_G = xi_C_G,
      ...
      s_C_G = s_C_G,
      CD8_total = CF + CE,
      CD8_functional_fraction = CF / pmax(CF + CE, EPS),
      CD8_exhausted_fraction = CE / pmax(CF + CE, EPS),
      CD8_activation_fraction = I / (as.numeric(pars["K_C"]) + I),
      CD8_exhaustion_rate = chi_C_G * I / (as.numeric(pars["K_chi"]) + I),
      CD8_killing_rate = k_C_G * CF,
      p_eff = p_G / (1 + xi_C_G * CF),
...
make_start_vector <- function(model_flags, start_free_pars = default_start_free_pars) {
  ...
  x0 <- c(
    ...
    log_chi_C = log(unname(start_free_pars["chi_C"]))
  )
  ...
  if (model_flags["zchi"] == 1) {
    x0 <- c(x0, theta_chi = unname(start_free_pars["theta_chi"]))
  }
...
unpack_pars <- function(x, model_flags) {
  ...
  if (model_flags["zchi"] == 1) {
    pars["theta_chi"] <- unname(x["theta_chi"])
  }
  ...
  print(
    sim %>%
      summarise(
        ...
        min_CE = min(CE, na.rm = TRUE),
        max_CE = max(CE, na.rm = TRUE),
        min_CD8_total = min(CD8_total, na.rm = TRUE),
        max_CD8_total = max(CD8_total, na.rm = TRUE),
...
fit_one_model <- function(
  ...
  model_flags <- c(
    ...
    zchi = as.numeric(model_row$zchi)
  )
  ...
  chi_C_late <- unname(pars_hat["chi_C"])
  chi_C_early <- unname(pars_hat["chi_C"]) *
    exp(unname(model_flags["zchi"]) * unname(pars_hat["theta_chi"]))
  ...
  param_row <- tibble(
    ...
    theta_chi = unname(pars_hat["theta_chi"]),
    a_L_late = a_L_late,
    a_L_early = a_L_early,
    k_C_late = k_C_late,
    k_C_early = k_C_early,
    ...
    chi_C_late = chi_C_late,
    chi_C_early = chi_C_early,
    ...
    ratio_chi_C_early_late = chi_C_early / chi_C_late,
    start_id = start_id
  )
...
make_random_start <- function() {
  ...
  c(
    ...
    k_C = exp(runif(1, log(1e-10), log(1e-5))),
    xi_C = exp(runif(1, log(1e-10), log(1e-5))),
    rho_C = exp(runif(1, log(1e-4), log(1))),
    chi_C = exp(runif(1, log(1e-5), log(1))),
    theta_a = runif(1, -2, 2),
    theta_k = runif(1, -2, 2),
    theta_xi = runif(1, -2, 2),
    theta_rho = runif(1, -2, 2),
    theta_chi = runif(1, -2, 2)
  )
}
...
params_row_to_start_free <- function(param_row) {
  ...
  c(
    ...
    chi_C = as.numeric(param_row$chi_C),
    ...
    theta_chi = ifelse(is.na(param_row$theta_chi), 0, as.numeric(param_row$theta_chi))
  )
}
...
is_nested_parent <- function(parent_row, child_row) {
  ...
  parent_flags <- c(
    ...
    zchi = as.numeric(parent_row$zchi)
  )
  ...
  child_flags <- c(
    ...
    zchi = as.numeric(child_row$zchi)
  )
...
if (RUN_SINGLE_SIM_TEST) {
  ...
  cat("\nSingle-subject total-CD8 exhaustion model check:\n")
  print(
    sim_test %>%
      summarise(
        id = first(id),
        ...
        CF0 = first(CF0),
        CE0 = first(CE0),
        median_CD8_total = median(CD8_total, na.rm = TRUE),
        median_CF = median(CF, na.rm = TRUE),
        median_CE = median(CE, na.rm = TRUE),
        median_functional_fraction = median(CD8_functional_fraction, na.rm = TRUE),
        median_exhausted_fraction = median(CD8_exhausted_fraction, na.rm = TRUE),
  ...
  p_sim_test <- sim_test %>%
    pivot_longer(
      cols = c(T, L, I, V, CF, CE, CD8_total, CD4_total),
      names_to = "state",
      values_to = "value"
    ) %>%
...
# ============================================================
# 25. Anchored random multi-start fitting for all candidate models
# ============================================================
      ...
      if (is_nested_parent(parent_row_j, model_row_i)) {
        ...
        if (
          ...
          if (
            ...
            all(anchor_start_j[c("a_L", "p", "k_C", "xi_C", "rho_C", "chi_C")] > 0)
          ) {
...
boundary_check <- best_fit_params_ms %>%
  mutate(
    ...
    k_C_near_lower = !is.na(k_C) & k_C <= 1.01e-12,
    k_C_near_upper = !is.na(k_C) & k_C >= 0.99e-4,
    xi_C_near_lower = !is.na(xi_C) & xi_C <= 1.01e-12,
    xi_C_near_upper = !is.na(xi_C) & xi_C >= 0.99e-4,
    rho_C_near_lower = !is.na(rho_C) & rho_C <= 1.01e-6,
    rho_C_near_upper = !is.na(rho_C) & rho_C >= 4.95,
    chi_C_near_lower = !is.na(chi_C) & chi_C <= 1.01e-8,
    chi_C_near_upper = !is.na(chi_C) & chi_C >= 4.95,
    ...
    theta_chi_near_boundary = !is.na(theta_chi) & abs(theta_chi) >= 4.95
  )
...
if (
  any(
    boundary_check$a_L_near_lower |
      ...
      boundary_check$chi_C_near_lower |
      boundary_check$chi_C_near_upper |
      ...
      boundary_check$theta_chi_near_boundary,
    na.rm = TRUE
  )
) {
  warning(
...
if (!is.null(best_fit_overall$fit)) {
  ...
  best_model_flags <- c(
    ...
    zchi = as.numeric(best_model_row$zchi)
  )
  ...
  cd8_effect_summary <- best_sim %>%
    ...
    summarise(
      CD8_0 = first(CD8_0),
      CD8_0_source = first(CD8_0_source),
      median_CF = median(CF, na.rm = TRUE),
      median_CE = median(CE, na.rm = TRUE),
      median_CD8_total = median(CD8_total, na.rm = TRUE),
      median_functional_fraction = median(CD8_functional_fraction, na.rm = TRUE),
      ...
      median_CD8_exhaustion_rate = median(CD8_exhaustion_rate, na.rm = TRUE),
      median_CD8_killing_rate = median(CD8_killing_rate, na.rm = TRUE),
      median_p_eff_over_p_G = median(p_eff_over_p_G, na.rm = TRUE),
      ...
      chi_C_G = median(chi_C_G, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    arrange(group, id)
...
run_settings <- tibble(
  ...
  value = c(
    ...
    "../Rdata/raw_data_with_cd8_counts.csv",
    "../Rdata/init_table_I0_replaced_with_CD8.csv",
    as.character(FRAC_E0_FIXED),
    ...
    as.character(fixed_pars["d_F"]),
    as.character(fixed_pars["d_E"]),
    as.character(fixed_pars["K_C"]),
    as.character(fixed_pars["K_chi"]),
    ...
    "CD8 raw cells/mL converted to log10; model prediction log10(CF + CE)",
  ...
  best_param_plot_data <- best_fit_overall$params %>%
    ...
    mutate(
      parameter = case_when(
        ...
        grepl("^chi_C", parameter_group) ~ "chi_C",
...
if (exists("best_sim") && !is.null(best_sim)) {
  ...
  cd8_timecourse_plot_data <- best_sim %>%
    select(
      ...
      CE,
      CD8_total,
      CD8_functional_fraction,
      CD8_exhausted_fraction,
      CD8_killing_rate,
      p_eff_over_p_G,
      CD8_exhaustion_rate
    ) %>%
...
cat("2. init_table_with_CD8_used.csv\n")
