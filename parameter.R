...
# ============================================================
# Selected fixed-parameter comparison for the new CD8 model
# ============================================================
# Purpose:
#   Test only several selected fixed-parameter sets under the new model.
...
#   C_scaled = C / 1e6
#
# For each selected fixed-parameter set, this script fits:
#   a_L, p, k_C, xi_C, rho_C
#
# Fixed parameters compared:
#   S03, S05, S10
# ============================================================
...
out_dir <- file.path(
  DATA_DIR,
  "selected_parameter_sets_new_CD8_model",
  run_tag
)
...
fixed_parameter_grid <- tribble(
  ...
  "S03",      "classic_delta_c23",        0.01, 1.58e-8,   0.005, 5e-4,  0.45,    23,  0.75, 1e4,   5.0,     CD8_SCALE,
...
write.csv(
  ...
  file.path(out_dir, "selected_fixed_parameter_sets.csv"),
  row.names = FALSE
)
...
selected_parameter_ranking <- parameter_fit_table %>%
...
print(selected_parameter_ranking)

write.csv(
  selected_parameter_ranking,
  file.path(out_dir, "selected_parameter_ranking.csv"),
  row.names = FALSE
)

write.csv(
  best_fit_params,
  file.path(out_dir, "selected_parameter_best_fit_free_params.csv"),
  row.names = FALSE
)

write.csv(
  parameter_fit_table,
  file.path(out_dir, "selected_parameter_fit_summary.csv"),
  row.names = FALSE
)

best_by_problem_score <- selected_parameter_ranking %>%
  arrange(problem_score, RSS) %>%
  slice(1)

best_by_RSS <- selected_parameter_ranking %>%
  arrange(RSS) %>%
  slice(1)

best_mainline_candidate <- selected_parameter_ranking %>%
...
p_rank_problem <- selected_parameter_ranking %>%
  ...
  theme_bw() +
  labs(
    title = "Selected fixed-parameter sets ranked by problem score",
    x = "Fixed-parameter setting",
    y = "Problem score = weighted RSS + pVL/CD8 bias penalty"
  )

print(p_rank_problem)
save_plot_png_pdf(p_rank_problem, "Fig1_selected_parameter_ranking_problem_score", width = 9, height = 5)

p_rank_RSS <- selected_parameter_ranking %>%
  mutate(setting_label = paste(setting_id, description, sep = ": ")) %>%
  mutate(setting_label = factor(setting_label, levels = rev(setting_label))) %>%
  ...
  theme_bw() +
  labs(
    title = "Selected fixed-parameter sets ranked by weighted RSS",
    x = "Fixed-parameter setting",
    y = "Weighted RSS"
  )

print(p_rank_RSS)
save_plot_png_pdf(p_rank_RSS, "Fig2_selected_parameter_ranking_RSS", width = 9, height = 5)
...
cat("\nSelected fixed-parameter comparison finished.\n")
...
cat("\nMain files to check:\n")
cat("1. selected_parameter_ranking.csv\n")
...
cat("1. Fig1_selected_parameter_ranking_problem_score.png\n")
cat("2. Fig2_selected_parameter_ranking_RSS.png\n")
